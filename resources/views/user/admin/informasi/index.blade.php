...
@push('script')
    ...
    <script>
        ...
        function read() {
            $.get("{{ url('perindag/informasi/read') }}", {}, function(data, status) {
                $("#myTable").html(data);
            });
        }
        ...
        function addData() {
            $.ajax({
                method: 'get',
                url: "{{ url('perindag/informasi/create') }}",
        ...
        function save() {
            $('#formPost').on('submit', function(e) {
                ...
                $.ajax({
                    method: 'post',
                    url: "{{ url('perindag/informasi/store') }}",
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function(res) {
        ...
        function edit(id) {
            if (id) {
                if (confirm('Are you sure want to update this post') === true) {
                    $.ajax({
                        method: 'get',
                        url: `{{ url('perindag/informasi/edit/${id}') }}`,
                        success: function(res) {
                            $('#modal').find('.modal-dialog').html(res)
                            modal.show()
                        }
        ...
        function ubah(id) {
            $('#formUpdate').on('submit', function(e) {
                ...
                $.ajax({
                    method: 'post',
                    url: "{{ url('perindag/informasi/ubah') }}" + '/' + id,
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function(res) {
        ...
        function hapus(id) {
            if (id) {
                if (confirm('Are you sure want to delete this post') === true) {
                    $.ajax({
                        method: 'get',
                        url: "{{ url('perindag/informasi/hapus') }}" + '/' + id,
                        success: function(res) {
                            read()
