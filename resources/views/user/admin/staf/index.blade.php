    ...
    <section class="section">
        <div class="card">
            <div class="card-header">
                <a href="{{ url('perindag/staf/create') }}" class="btn btn-outline-primary float-end"><i
                            ...
                            @foreach ($users as $user)
                                @if ($user->role == 2)
                                    <tr>
                                        ...
                                        <td>
                                            <form action="{{ url('perindag/staf/' . $user->id) }}" method="post"
                                                onsubmit="return confirm('Apakah anda yakin ingin menghapus data ini ?');">
                                                @csrf
                                                @method('DELETE')
                                                <a href="{{ url('perindag/staf/' . $user->id . '/edit') }}"
